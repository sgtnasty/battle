...
namespace BattleEngine
{
	public class RandomName
	{
		public RandomName (Random random)
		{
			...
			string stack = @"Abban Adomn Adhamh Adhamhnán Adamnan Eunan Adanodan Ailbhe Ailbe Alby 
...
Vran Vron Wannard Weonard";
			this.names = stack.Split (new char[] {' ', '\n'});
		}
		...
		{
			int max = this.names.Length;
			string name = "";
			while (name.Length < 1)
				name = this.names[this.random.Next (0, max)];
			return name;
		}
	}
}
